...
import HeaderLogo from '../../../public/niagara_site_logotipo_300x75.png';
...
const navItems = [
  ...
  { name: 'Por que nos escolher?', href: '#why-chose-us' },
...
const Header: React.FC = () => {
  ...
  return (
    <header className="fixed div-header w-full bg-[#274651] z-49 top-0 left-0 ">
      <div className="flex mx-auto items-center justify-between  h-16 px-6">
        {/* Logo/Título da Empresa */}
        <div className="text-white text-2xl font-bold flex w-max ml-[-2rem]">
          <Link href={"#main"}>
            <img
              src={HeaderLogo.src}
              alt="Logo da Empresa"
              className="max-w-xs md:max-w-sm lg:max-w-md h-[3.5rem]"
            />
          </Link>
        </div>
          ...
          <ul className="flex space-x-6 text-white font-medium uppercase">
            {navItems.map((item) => (
              <li key={item.name}>
                <Link
                  href={item.href}
                  className="hover:text-[#009dc1] transition-colors duration-300 font-bold"
                >
                  {item.name}
                </Link>
              </li>
        ...
        <nav
          className={`
            fixed top-0 right-0 h-full w-64 bg-[#274651] shadow-xl p-6
          ...
          <ul className="flex flex-col space-y-4 text-white font-medium uppercase">
            {navItems.map((item) => (
              <li key={item.name}>
                <Link
                  href={item.href}
                  className="block py-2 hover:text-[#009dc1] transition-colors duration-300 border-b border-gray-700"
